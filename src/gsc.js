...
export async function getProperties(request, env) {
    ...
    const accessToken = await env.AUTH_STORE.get(`gsc_token:${userId}`);
  ...
  export async function fetchGSCData(request, env) {
    ...
    if (!accessToken) {
      // Handle token refresh as in getProperties
      // ...
    }
  ...
  export async function getTopPages(request, env) {
    ...
    // Create GSC request with custom parameters
    // Similar to fetchGSCData, but specifically for pages
    // ...
